...
with open("resultats-ingredients.csv", "w", encoding='utf-8', newline="") as f:
    ...
    for url in tqdm(urls):
        ...
        # Liste Ingrédients
        list_ing = soup.find_all("div", {"class": "gallery-cell"})  # Recherche tous les éléments avec la classe "gallery-cell"
        list_ing_result = []  # Initialisation de la liste des résultats
        # print(len(list_ing))
        for element in list_ing:
            # Pour chaque élément avec la classe "gallery-cell", recherche les éléments avec les classes "titre", "images" et "description"
            nom_latin = element.find("div", {"class": "nom_latin"}).text.strip() if element.find("div", {"class": "nom_latin"}) else 'none'
            try :
                photo_ingredient = element.find("div", {"class": "photo_ingredient"}).find("i")["aria-label"] if element.find("div", {"class": "photo_ingredient"}).find("i")["aria-label"] else 'none'
            except AttributeError:
                print(f"Warning: photo_ingredient not found for URL {url}")
                continue
            try :
                fonction_ingredient = element.find("div", {"class": "fonction_ingredient"}).text.strip() if element.find("div", {"class": "fonction_ingredient"}) else 'none'
            except AttributeError:
                print(f"Warning: fonction_ingredient not found for URL {url}")
                continue
            try :
                cell_ribbon = element.find("div", {"class": "cell-ribbon"}).text.strip() if element.find("div", {"class": "cell-ribbon"}) else 'none'
            except AttributeError:
                print(f"Warning: label not found for URL {url}")
                continue
            list_ing_result.append([cell_ribbon,  nom_latin, fonction_ingredient, photo_ingredient])
